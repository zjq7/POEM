...
pip install -r req.txt # 这里是我搭建好的环境后的所有包的版本, 可以直接使用
pip install -e ./ # 安装当前文件中的tf2-gnn到环境中
```
- 测试命令
`python train.py RGCN PPI /mnt/e/08_tfGnnSample/data/ppi`

2020/12/4
- 解决二分类问题准确率一直为1的问题
```python
# tf2-gnn\tf2_gnn\data\jsonl_graph_property_dataset.py
        if self._threshold_for_classification is not None:
            # add by zjq
            # 这里是对数据集标签的判断,因为是2分类, 所以直接那个啥就行了
...
"num_fwd_edge_types": 10 # 这里虽然是11条边的输入, 但是要少写一条
```
- 增加tf2将tensor格式的embedding打印一下
```python
# tf2_gnn\models\graph_regression_task.py
        # add by zjq
        # 这里希望得到图网络倒数第二层的embedding
        # tf.Tensor([1 2 3 4 5 6 7 8 9 0], shape=(10,), dtype=int32)
        # tensor结果是: Tensor("graph_binary_classification_task/concat_1:0", shape=(None, 64), dtype=float32)
        print("tensor结果是:",graph_representations)
        tf.print(graph_representations, output_stream="file://graph_representations.txt", summarize=-1)
        # 该函数返回一个张量，这个张量是将原始input中所有维度为1的那些维都删掉的结果
```
2020/12/21
- 训练指令
`python train.py RGIN GraphBinaryClassification /mnt/e/08_tfGnnSample/data/ast/`

- load模型测试指令
`python train.py RGIN GraphBinaryClassification /mnt/e/08_tfGnnSample/data/ast/ --load-saved-model trained_model/RGIN_GraphBinaryClassification__2020-12-11_10-50-13_best.pkl --run-test`
